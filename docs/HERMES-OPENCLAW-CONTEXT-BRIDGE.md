...
### 3. 分工边界（强制口径）
- **OpenClaw = executor**：本地开发、改代码、跑命令、做部署、拿运行证据。
- **Hermes = planner / reviewer / supervisor**：定义验收标准、审阅变更、把控 release gate、记录回滚点、部署后监督健康状态。
- **GitHub = code SSOT**：代码、脚本、版本、PR、tag、changelog。
- **Server = runtime SSOT**：服务进程、环境变量、日志、运行态状态文件。

原则：
1. OpenClaw 不以“聊天已说过”代替文档落库。
2. Hermes 不直接把服务器热修当成长期事实源。
3. 任何关键 handoff（本地→GitHub、GitHub→服务器、服务器→验收）都要可追溯。
...
### 5. 部署后 Hermes 监督项
每次部署完成后，Hermes 应监督并记录：
- 服务是否正常运行
- `/health` 与核心页面是否可达
- 首屏/核心流程是否异常
- 是否有静态资源版本错配
- 是否出现新的错误日志/异常
- 当前 rollback 目标是否明确可用
