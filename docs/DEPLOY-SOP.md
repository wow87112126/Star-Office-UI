...
- 本地预览与实验
- 文件修改、脚本编排、部署执行
- 线上 smoke check 与故障定位

### Hermes：规划 / 审核 / 监督者
负责：
- 维护共享 SSOT 文档与任务状态
- 在发布前审阅变更范围、风险、验收标准
- 在部署前确认 release gate 是否通过
- 在部署后监督健康检查、日志、回滚准备度
- 把关键结论沉淀回 `docs/` 与 `memory/`

### GitHub：代码事实源
负责：
- 代码、脚本、文档、版本记录
- commit / branch / tag / PR 审阅轨迹

### 服务器：运行事实源
负责：
- runtime 进程、环境变量、日志、临时状态

原则：
- **代码真源在 GitHub，不在服务器热修文件里。**
- **运行状态真源在服务器，不在聊天里。**
- **项目规则真源在工作区共享 docs，不在单个 agent 记忆里。**
...
## 八、当前线上维护 SOP（代码层）
1. OpenClaw 在本地修改：`C:\Users\Administrator\.openclaw\workspace\Star-Office-UI`
2. OpenClaw 本地验证改动
3. Hermes 审阅本次变更范围、风险、文档同步项
4. 代码 commit 并 push 到 GitHub fork
5. 记录：
   - deploy commit
   ...
   - 回滚方式
6. 服务器备份对应文件/目录
7. 上传覆盖到：`/home/ecs-user/Star-Office-UI`
8. 重启服务：
   ```bash
...
### 原则
- 优先通过重新部署上一个稳定 commit 回滚
- 如果线上做过热修，必须尽快回写 GitHub，不允许长期漂移
- 若未来出现不可逆迁移，必须在部署前先做快照/备份
...
1. **把“本地开发 → GitHub fork → 服务器”的发布链跑顺**
2. **再把本地 OpenClaw 的实时状态，安全地同步到公网 Star Office UI**

前者解决长期维护与回滚，后者解决“宠物动作是否真的像本地一样实时联动”。
